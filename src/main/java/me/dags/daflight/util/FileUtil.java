...
public class FileUtil
{
    ...
    public static File createFile(File fileIn)
    {
        if (!fileIn.exists())
        {
            try
            {
                if (fileIn.createNewFile()) logger.info("Creating new file: " + fileIn);
            }
            catch (IOException e)
            {
                e.printStackTrace();
    ...
    public static File createFolder(File parent, String... subDirs)
    {
        for (String dir : subDirs)
        {
            ...
            if (parent.mkdirs()) logger.info("Creating new folder: " + parent);
        }
        return parent;
    }
    ...
    {
        if (!folder.exists()) if (folder.mkdirs()) logger.info("Creating folder: " + folder);
    ...
    public static boolean serialize(Object in, File toFile)
    {
        if (in != null && toFile != null)
        {
            try
            {
                FileWriter fw = new FileWriter(createFile(toFile));
